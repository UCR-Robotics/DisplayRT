...
myPublisherROS2Humble::myPublisherROS2Humble()
: Node("myPublisherROS2Humble")
{
    this->publisher_sensors_ = this->create_publisher<displayrt_example_ros2humble::msg::Sensors>(this->channel_sensors_, 10);
    this->publisher_servos_ = this->create_publisher<displayrt_example_ros2humble::msg::Servos>(this->channel_servos_, 10);

    // Capture the start time
    start_time_ = std::chrono::steady_clock::now();
}

void myPublisherROS2Humble::publishServos()
{
    const auto timestamp = this->getElapsedTimeInMilliseconds();

    auto message = displayrt_example_ros2humble::msg::Servos();
    ...
    {
        auto servo_message = displayrt_example_ros2humble::msg::RevoluteServo();
...
void myPublisherROS2Humble::publishSensors()
{
    const auto timestamp = this->getElapsedTimeInMilliseconds();

    auto message = displayrt_example_ros2humble::msg::Sensors();
    message.timestamp = timestamp;
    ...
    {
        auto imu_message = displayrt_example_ros2humble::msg::IMU();
...
mySubscriberROS2Humble::mySubscriberROS2Humble()
: Node("mySubscriberROS2Humble")
{
    this->subscription_servos_ = this->create_subscription<displayrt_example_ros2humble::msg::Servos>(
        this->channel_servos_, 10, std::bind(&mySubscriberROS2Humble::servosCallbackHandler, this, std::placeholders::_1)
    );
    this->subscription_sensors_ = this->create_subscription<displayrt_example_ros2humble::msg::Sensors>(
        this->channel_sensors_, 10, std::bind(&mySubscriberROS2Humble::sensorsCallbackHandler, this, std::placeholders::_1)
    );
}

void mySubscriberROS2Humble::servosCallbackHandler(const displayrt_example_ros2humble::msg::Servos::SharedPtr msg)
{
    this->servos_data_ = std::make_shared<displayrt_example_ros2humble::msg::Servos>(*msg);
...
void mySubscriberROS2Humble::sensorsCallbackHandler(const displayrt_example_ros2humble::msg::Sensors::SharedPtr msg)
{
    this->sensors_data_ = std::make_shared<displayrt_example_ros2humble::msg::Sensors>(*msg);
...
myDisplayRT_ROS2Humble::myDisplayRT_ROS2Humble( const std::shared_ptr<DisplayRT_Property> monitor_property )
: DisplayRT(monitor_property), Node("myDisplayRT_ROS2Humble")
{
    this->subscription_servos_ = this->create_subscription<displayrt_example_ros2humble::msg::Servos>(
        this->channel_servos_, 10, std::bind(&myDisplayRT_ROS2Humble::servosCallbackHandler, this, std::placeholders::_1)
    );
    this->subscription_sensors_ = this->create_subscription<displayrt_example_ros2humble::msg::Sensors>(
        this->channel_sensors_, 10, std::bind(&myDisplayRT_ROS2Humble::sensorsCallbackHandler, this, std::placeholders::_1)
    );

    this->servos_data_ = std::make_shared<displayrt_example_ros2humble::msg::Servos>();
    this->sensors_data_ = std::make_shared<displayrt_example_ros2humble::msg::Sensors>();
}

DisplayRT::Status myDisplayRT_ROS2Humble::Setup()
{
    // DisplayRT::defaultSetupDisplay();

    // QObject::connect( this, &myDisplayRT_ROS2Humble::servosCallbackHandler, this, &myDisplayRT_ROS2Humble::Update );

    QTimer *dataTimer = new QTimer( this->_app.get() );
    QObject::connect( dataTimer, &QTimer::timeout, this, &myDisplayRT_ROS2Humble::Update );
    dataTimer->start(50); // Update every 100 ms

    return Status::NORMAL;
}    

DisplayRT::Status myDisplayRT_ROS2Humble::Update()
{
    if( rclcpp::ok() )
    {
    ...
    { // servos
        { // integrity check
            // if ( this->servos_data_.get() == nullptr )
            // {
            //     throw std::runtime_error("[WARNING] myDisplayRT_ROS2Humble: servos data is null");
            // }
            // compare channel name
            if ( this->_monitor_property->WindowProperties()[0]->Channel() != this->channel_servos_ )
            {
                std::stringstream ss;
                ss << "[WARNING] myDisplayRT_ROS2Humble: channel name mismatch, expected " << this->_monitor_property->WindowProperties()[0]->Channel() << " but got " << this->channel_servos_ << std::endl;
            ...
            {
                std::stringstream ss;
                ss << "[WARNING] myDisplayRT_ROS2Humble: plot count mismatch, expected " << this->_monitor_property->WindowProperties()[0]->PlotCount() << " but got " << this->servos_data_->revolute_servo_count << std::endl;
                throw std::runtime_error(ss.str());
            }
        }
        ...
        { // plotting
            ...
            { // plot per servo
                ...
                { // integrity check
                    ...
                    {
                        std::stringstream ss;
                        ss << "[WARNING] myDisplayRT_ROS2Humble: plot " << plot->Id() << " has invalid axis count [" << plot->Axes().size() << "]\n";
                        throw std::runtime_error(ss.str());
                    }
                }
                ...
                { // position
                    ...
                    { // integrity check
                        ...
                        {
                            std::stringstream ss;
                            ss << "[WARNING] myDisplayRT_ROS2Humble: plot " << plot->Id() << " axis " << axis->Id() << " has invalid graph count [" << axis->Axis()->graphs().size() << "]\n";
                            throw std::runtime_error(ss.str());
                        }
                    }
    ...
    { // sensors
        { // integrity check
            // if ( this->sensors_data_.get() == nullptr )
            // {
            //     throw std::runtime_error("[WARNING] myDisplayRT_ROS2Humble: sensors data is null");
            // }
            // compare channel name
            if ( this->_monitor_property->WindowProperties()[1]->Channel() != this->channel_sensors_ )
            {
                std::stringstream ss;
                ss << "[WARNING] myDisplayRT_ROS2Humble: channel name mismatch, expected " << this->_monitor_property->WindowProperties()[1]->Channel() << " but got " << this->channel_sensors_ << std::endl;
                throw std::runtime_error(ss.str());
            }
            ...
            {
                std::stringstream ss;
                ss << "[WARNING] myDisplayRT_ROS2Humble: plot count mismatch, expected " << this->_monitor_property->WindowProperties()[1]->PlotCount() << " but got " << this->sensors_data_->imu_sensor_count << std::endl;
                throw std::runtime_error(ss.str());
            }
        }
...
void myDisplayRT_ROS2Humble::servosCallbackHandler(const displayrt_example_ros2humble::msg::Servos::SharedPtr msg)
{
    this->servos_data_ = std::make_shared<displayrt_example_ros2humble::msg::Servos>(*msg);
...
void myDisplayRT_ROS2Humble::sensorsCallbackHandler(const displayrt_example_ros2humble::msg::Sensors::SharedPtr msg)
{
    this->sensors_data_ = std::make_shared<displayrt_example_ros2humble::msg::Sensors>(*msg);
